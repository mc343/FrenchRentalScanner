...
class BaseScraper(ABC):
    ...
    def normalize_listing_data(self, listing: Dict) -> Dict:
        ...
        listing.update({
            ...
            'has_elevator': listing.get('has_elevator', False) or ('elevator' in feature_blob or 'ascenseur' in feature_blob),
            'has_parking': listing.get('has_parking', False) or ('parking' in feature_blob),
            'has_balcony': listing.get('has_balcony', False) or ('balcony' in feature_blob or 'balcon' in feature_blob),
            'has_garden': listing.get('has_garden', False) or ('garden' in feature_blob or 'jardin' in feature_blob),
            'furnished': listing.get('furnished', False) or ('furnished' in feature_blob or 'meubl' in feature_blob),
        })

        if not listing.get('available_date'):
            listing['available_date'] = self.extract_available_date(" ".join([title, description, " ".join(features)]))
        else:
    ...
    def extract_available_date(self, text: str) -> Optional[datetime]:
        ...
        date_match = re.search(r'(?:disponible|available)\s+(?:à partir du|a partir du|from)?\s*(\d{1,2})[\/\-](\d{1,2})(?:[\/\-](\d{2,4}))?', normalized)
        if date_match:
