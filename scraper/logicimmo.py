"""
Logic-Immo scraper - rental search via Logic-Immo pages.
"""
...
class LogicImmoScraper(BaseScraper):
    """Scraper for Logic-Immo rental listings."""
    ...
    def search(self, filters: Dict) -> List[Dict]:
        """Search Logic-Immo for rental listings."""
        ...
        listings = []
        max_results = int(self.config.get("max_results", 14))
    ...
    def parse_listing(self, url: str) -> Dict:
        ...
        if not description:
            description = self._collect_json_ld_value(soup, "description")
        ...
        listing = {
            ...
            "source": self.name,
            "url": url,
            "title": self.clean_text(title) or "Logic-Immo rental",
    ...
    def _extract_listing_urls(self, soup: BeautifulSoup) -> List[str]:
        """Extract detail links from Logic-Immo result pages."""
        urls = []
        seen = set()

        selectors = [
            'a[href*="/detail-location-"]',
            'a[href*="/location-appartement-"]',
            'a[href*="/location-maison-"]',
        ]
        ...
        for data in self._iter_json_ld_objects(soup):
            for candidate in self._collect_urls(data):
                if "/location-" in candidate and candidate not in seen:
                    seen.add(candidate)
    ...
    def _collect_urls(self, payload) -> List[str]:
        """Collect URLs recursively from JSON-LD."""
        urls: List[str] = []
        if isinstance(payload, dict):
            for key, value in payload.items():
                if key in {"url", "@id"} and isinstance(value, str):
                    urls.append(value)
                else:
    ...
    def _collect_json_ld_value(self, soup: BeautifulSoup, key_name: str) -> str:
        """Find a recursive value inside JSON-LD blobs."""
        for data in self._iter_json_ld_objects(soup):
            found = self._find_value(data, key_name)
            if found:
                return str(found)
        return ""

    def _find_value(self, payload, key_name: str):
        """Recursively find a value by key."""
        if isinstance(payload, dict):
            for key, value in payload.items():
                if key == key_name and value:
                    return value
                found = self._find_value(value, key_name)
                if found:
                    return found
        elif isinstance(payload, list):
            for item in payload:
                found = self._find_value(item, key_name)
                if found:
                    return found
        return None

    def _extract_listing_id(self, url: str) -> str:
        """Extract a stable listing id from URL."""
        match = re.search(r"(\d{8,12})", str(url or ""))
        if match:
    ...
    def _extract_location(self, text: str) -> str:
        """Extract a location segment from page text."""
        match = re.search(r"([A-Za-zÀ-ÿ\-' ]+\(\d{5}\))", text)
        if match:
            return self.clean_text(match.group(1))
        return ""

    def _extract_city(self, text: str) -> str:
        """Extract the city token."""
        match = re.search(r"\b(Mulhouse|Huningue)\b", text, re.IGNORECASE)
        return match.group(1).title() if match else ""

    def _extract_property_type(self, title: str, text: str) -> str:
        """Infer apartment vs house."""
